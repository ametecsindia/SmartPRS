...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .list{width:340px;flex-shrink:0} .detail{flex:1;min-height:200px}
  ...
  .toast{position:fixed;left:50%;bottom:24px;transform:translateX(-50%);background:var(--navy);color:#fff;padding:11px 18px;border-radius:10px;font-size:13px;opacity:0;transition:opacity .2s;z-index:60} .toast.show{opacity:1}
  ...
  .rr{display:flex;gap:10px} .rr>div{flex:1}
...
<div class="mov" id="inviteMov"><div class="modal">
  ...
  <p class="s">Issue a Temp-EMP ID and send the onboarding link.</p>
  <label>Type</label>
  <select id="ivMode"><option value="new">New candidate</option><option value="existing">Existing employee</option></select>
  <label id="ivNameLbl">Full name</label><input id="ivName" placeholder="Candidate name">
  <div id="ivNewFields"><div class="rr"><div><label>Email</label><input id="ivEmail" placeholder="name@email.com"></div><div><label>Mobile / WhatsApp</label><input id="ivMobile" placeholder="+91…"></div></div></div>
  <div id="ivHint" class="s" style="display:none">Enter the employee’s <b>code or name</b> — details pre-fill from their record.</div>
  <div id="ivResult"></div>
...
(function(){
  ...
  function loadList(){
    api('{{ route('app.selfonboard.list') }}').then(function(r){
      ...
      if(!r.rows.length){host.innerHTML='<div class="empty">No submissions yet. Use “+ Invite”.</div>';return;}
      host.innerHTML=r.rows.map(function(x){return '<div class="row" data-id="'+x.id+'"><div class="nm">'+esc(x.name||'—')+' '+pill(x.status)+'</div><div class="meta"><span>'+esc(x.temp_emp_code)+'</span><span>E '+(x.email_verified?'✔':'—')+' · M '+(x.mobile_verified?'✔':'—')+'</span><span>'+x.docs+' docs</span><span>'+(x.selfie?'selfie ✔':'no selfie')+'</span></div></div>';}).join('');
  ...
  function kvBlock(t,obj,map){if(!obj)obj={};return '<h3>'+t+'</h3>'+map.map(function(m){return '<div class="kv"><span class="k">'+m[1]+'</span><span class="v">'+esc(obj[m[0]]||'—')+'</span></div>';}).join('');}
  ...
  function renderDetail(rec){
    ...
    var html='<div class="dhead"><h2>'+esc(rec.name||'—')+'</h2><span class="code">'+esc(rec.temp_emp_code)+' · '+pill(rec.status)+'</span><div class="badges">'+badges+'</div></div>'+
      ...
      '<div class="corr" id="corrBox"><h3>Items to correct (one per line)</h3><textarea id="corrItems" rows="3" placeholder="e.g. Education certificate is unreadable"></textarea><h3 style="margin-top:10px">Note (optional)</h3><textarea id="corrNote" rows="2"></textarea><div style="margin-top:10px;display:flex;gap:10px"><button class="btn primary" id="sendCorr">Send &amp; notify</button><button class="btn ghost" id="cancelCorr">Cancel</button></div></div>';
  ...
  $('#ivMode').onchange=function(){var ex=this.value==='existing';$('#ivNewFields').style.display=ex?'none':'block';$('#ivHint').style.display=ex?'block':'none';$('#ivNameLbl').textContent=ex?'Employee code or name':'Full name';$('#ivName').placeholder=ex?'e.g. EMP0007 or Ravi Kumar':'Candidate name';};
  $('#ivSend').onclick=function(){
    var mode=$('#ivMode').value,val=$('#ivName').value.trim(),b=this;
    if(mode==='existing'){
      if(!val){toast('Enter an employee code or name');return;}
      b.disabled=true;b.textContent='Sending…';
      jpost('{{ route('app.selfonboard.invite.existing') }}',{emp:val}).then(function(r){b.disabled=false;b.textContent='Send invite';if(!r.ok){toast(r.error||'Could not invite');return;}$('#ivResult').innerHTML='<div class="okbox">✔ Invited <b>'+esc(r.name)+'</b> ('+esc(r.emp_code_existing)+') · '+esc(r.temp_emp_code)+'<br>Link: <a href="'+r.link+'" target="_blank">'+r.link+'</a></div>';toast('Invite sent');loadList();});
      return;
    }
    var email=$('#ivEmail').value.trim(),mobile=$('#ivMobile').value.trim();
    if(!val||(!email&&!mobile)){toast('Enter a name and an email or mobile');return;}
    b.disabled=true;b.textContent='Sending…';
    jpost('{{ route('app.selfonboard.invite') }}',{name:val,email:email,mobile:mobile,mode:'new'}).then(function(r){b.disabled=false;b.textContent='Send invite';if(!r.ok){toast(r.error||'Could not invite');return;}$('#ivResult').innerHTML='<div class="okbox">✔ Invited <b>'+esc(val)+'</b> · '+esc(r.temp_emp_code)+'<br>Link: <a href="'+r.link+'" target="_blank">'+r.link+'</a></div>';$('#ivName').value='';$('#ivEmail').value='';$('#ivMobile').value='';toast('Invite sent');loadList();});
  };
